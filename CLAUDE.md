...
- `services/incomings_report/` — Python/FastAPI hexagonal. Financial reporting service for hotel managers. Reads billing records directly from BillingDB (read-only SQLAlchemy connection), persists enriched records (7.5% tax + 5% TravelHub commission applied) into its own ReportsDB, and exposes four JWT-protected endpoints: GET /api/reports/incoming (sync + list), GET /api/reports/revenue-overview (monthly chart data), GET /api/reports/dashboard-metrics (executive KPIs), GET /api/reports/incoming/csv (UTF-8 BOM CSV download). No inter-service HTTP calls — uses two SQLAlchemy engines (ReportsBase for ReportsDB, BillingBase read-only for BillingDB).
...
- **Deploy Applications** (`deploy_apps.yml`): Manual workflow (workflow_dispatch) — runs tests for Python services, provisions infra stacks (ecs_cluster, container_registry, cognito, database), builds and pushes all service Docker images to ECR, deploys ECS services, and updates API Gateway.
  - Test matrix: `pms`, `auth`, `booking`, `booking-orchestrator`, `notifications`, `incomings-report` (Python services)
  - Build & Push matrix: `hello_world`, `pms`, `auth`, `booking`, `booking_orchestrator`, `notifications`, `incomings_report` (Python), `poc_properties` (Java), `pricing_engine`, `pricing_orchestator` (.NET), `travelhub` (Node.js/Angular frontend)
...
- `booking-orchestrator` is the synchronous entry point for the booking lifecycle. It forwards the gateway-injected `X-User-Id` header when calling `booking`, converts ISO dates to `dd/MM/yyyy` when calling `poc_properties /api/property/lock`, and publishes versioned domain events (`BOOKING_CREATED`, `BOOKING_APPROVED`, `BOOKING_CONFIRMED`, `BOOKING_REJECTED`, `BOOKING_CANCELLED`, `BOOKING_DATES_CHANGED`, `PAYMENT_CONFIRMED`) onto `notifications_queue` (best-effort). Create reservation saga: creates booking first, then locks; on lock failure it **deletes** the booking (not cancel) and returns 409 `property_unavailable`. Admin reject accepts both PENDING and CONFIRMED bookings. Cancel saga: cancels the booking, computes a time-based cancellation policy (free if >= 24h before check-in, 50% penalty otherwise), unlocks the property via `POST /api/property/unlock` (best-effort), publishes a `CANCEL` command to `billing_queue` if payment was made (best-effort), and includes `refund_amount`/`penalty_amount` in the `BOOKING_CANCELLED` notification event. A `GET /api/reservations/{booking_id}/cancellation-policy` endpoint lets travelers preview the refund/penalty before confirming cancellation.
- `notifications` subscribes to `notifications_queue` via a lifespan-managed async SQS long-polling task and sends emails via `smtplib` against the AWS SES SMTP relay. On each consumed message it also reads `/{project}/notifications/fcm-tokens` from SSM (`SsmTokenRegistry.get_tokens()`) and sends FCM push notifications to all registered device tokens for the event's `user_id` via `firebase-admin`. Push failures are logged as warnings and do not prevent SQS ack. Device tokens are registered via the `register_push_token.yml` GitHub Actions `workflow_dispatch` workflow (inputs: `user_id`, `fcm_token`, `environment`). Unknown schema versions or handler failures are *not* acked — SQS redrives them to `notifications_dlq` automatically.
...
- `incomings_report` reads billing records directly from BillingDB via a read-only async SQLAlchemy connection (no HTTP call to the billing service). DB credentials sourced from SSM `/final-project-miso/billing/db_*` as `BILLING_DB_*` env vars.
