...
**Routing**: `MessageDispatcher` matches on `(schema_version, type)` pairs. Today only `(1, BOOKING_CREATED)` is handled. Unknown types or schema versions cause the consumer to **not** delete the message — SQS will retry, and after `max_receive_count` attempts the message is redriven to `notifications_dlq`. This keeps poison messages visible to operators instead of silently dropped.
...
src/notifications/
├── domain/
│   ├── events.py          # BookingCreatedEvent (consumed)
│   └── exceptions.py
├── application/
