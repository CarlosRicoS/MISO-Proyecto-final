...
namespace PricingOrchestator.Controllers
{
	...
	public class PropertyController(IHttpClientFactory httpClientFactory) : ControllerBase
	{
		...
		public async Task<IResult> GetProperty([FromQuery] PropertyPriceRequest request)
		{
			...
			string pricingEngineUrl = $"{pricingServiceUrl}/api/propertyprice?guests={request.Guests}&dateInit={request.DateInit}&dateFinish={request.DateFinish}&discountCode={request.DiscountCode ?? string.Empty}&propertyId={request.PropertyId}";
